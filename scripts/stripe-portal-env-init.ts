...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "-h" || arg === "--help") {
      console.log(
        "Usage: tsx scripts/stripe-portal-env-init.ts [--env-file <path>]",
      );
      process.exit(0);
    }
...
function main() {
  ...
  for (const [key, fallbackValue] of ENV_KEYS) {
    ...
    const defaultValue =
      key in dynamicDefaults ? dynamicDefaults[key] : fallbackValue;
    missing.push(`${key}=${defaultValue}`);
  }

  if (missing.length === 0) {
    console.log(
      `No changes needed. All Stripe portal env keys already exist in ${envFile}.`,
    );
    return;
  }
