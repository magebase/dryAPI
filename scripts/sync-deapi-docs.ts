...
const deapiOpenApiDocumentPath = path.join(
  process.cwd(),
  "docs",
  "deapi-mirror",
  "articles",
...
function normalizeCustomerFacingBranding(value) {
  return value
    .replaceAll(
      "https://docs.deapi.ai/openapi.json",
      "https://dryapi.dev/openapi.json",
    )
    .replace(
      /https:\/\/docs\.deapi\.ai\/([^\s)]+)\.md/g,
      (_match, pathPart) => `https://dryapi.dev/docs/v1/${pathPart}`,
    )
...
function stripMintlifyComponents(source) {
  return source
    .replace(/<Card([^>]*)>([\s\S]*?)<\/Card>/gi, (_, attrs, body) =>
      convertCard(attrs, body),
    )
    .replace(/<Card([^>]*)\/>/gi, (_, attrs) => convertCard(attrs, ""))
    .replace(
      /<ResponseField([^>]*)>([\s\S]*?)<\/ResponseField>/gi,
      (_, attrs, body) => convertResponseField(attrs, body),
    )
    .replace(
      /<(Note|Info|Warning|Tip)[^>]*>([\s\S]*?)<\/(Note|Info|Warning|Tip)>/gi,
      (_, kind, body) => convertCallout(kind, body),
    )
    .replace(
      /<\/(?:CardGroup|Tabs|Tab|AccordionGroup|Accordion|Steps|CodeGroup)>/gi,
      "",
    )
    .replace(
      /<(?:CardGroup|Tabs|Tab|AccordionGroup|Accordion|Steps|CodeGroup)[^>]*>/gi,
...
function resolveDocsLink(target, sourcePath, docsPathSet) {
  ...
  if (/^https?:\/\//i.test(href)) {
    ...
    return docsPathSet.has(normalized)
      ? `/docs/${CURRENT_VERSION}${normalized}${hashSuffix}`
      : target;
  }
...
function replaceOpenApiFenceWithApiPage(source) {
  ...
  return source.replace(
    pattern,
    (_, heading, __fence, method, operationPath) => {
      const normalizedMethod = String(method).toLowerCase();
      const normalizedPath = String(operationPath).trim();
      const operations = JSON.stringify([
        { path: normalizedPath, method: normalizedMethod },
      ]);
...
function stripDocumentationIndex(content) {
  ...
  const result =
    content.substring(0, startOfBlock) + content.substring(endOfBlock);
...
function buildVersionLanding(entries) {
  ...
  for (const entry of entries) {
    if (
      entry.kind === "markdown" &&
      !sectionFirstPages.has(entry.sectionTitle)
    ) {
  ...
  const lines = [
    frontmatterBlock({
      title: `dryAPI Documentation (${CURRENT_VERSION})`,
  ...
  function ensureMeta(directory) {
    if (!metaByDirectory.has(directory)) {
      const title = directory
        ? directory === CURRENT_VERSION
          ? `${CURRENT_VERSION} (latest)`
          : toTitleCase(directory.split("/").at(-1))
        : undefined;
...
async function writeMetaFiles(manifestEntries) {
  ...
  for (const [directory, meta] of metaByDirectory.entries()) {
    const targetDir = directory
      ? path.join(docsContentRoot, directory)
      : docsContentRoot;
...
async function writeDocsContent(manifestEntries, markdownByPath, docsPathSet) {
  ...
  for (const entry of manifestEntries) {
    ...
    await fs.writeFile(
      targetPath,
      toFumadocsMarkdown(source, entry, docsPathSet, brand),
      "utf8",
    );
  }

  await fs.writeFile(
    path.join(docsVersionRoot, "openapi.mdx"),
    toManualOpenApiPage(),
    "utf8",
  );
  await fs.writeFile(
    path.join(docsVersionRoot, "index.mdx"),
    buildVersionLanding(manifestEntries),
    "utf8",
  );
  await fs.writeFile(
...
async function writeContentEntries(entries) {
  ...
  const docsPathSet = new Set(
    manifestEntries
      ...
      .map((entry) =>
        normalizePathname(entry.sourcePath.replace(/\.md$/i, "")),
      ),
  );
...
async function main() {
  ...
  const markdownCount = manifestEntries.filter(
    (entry) => entry.kind === "markdown",
  ).length;
  const openApiCount = manifestEntries.filter(
    (entry) => entry.kind === "openapi",
  ).length;

  console.log(
    `Synced ${manifestEntries.length} docs entries (${markdownCount} markdown, ${openApiCount} openapi).`,
  );
