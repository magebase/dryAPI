...
function normalizeCustomerFacingBranding(value) {
  return String(value)
    ...
    .replaceAll("dryAPI.ai", "dryAPI")
    .replaceAll("dryAPI", "dryAPI")
...
function normalizeCustomerFacingSnapshot(snapshot) {
  return {
    ...
    sourceUrls: Array.isArray(snapshot.sourceUrls)
      ? snapshot.sourceUrls.map((value) =>
          normalizeCustomerFacingBranding(value),
        )
      : snapshot.sourceUrls,
      ...
      ? snapshot.permutations.map((permutation) => ({
          ...
          metadata: permutation.metadata
            ? {
                ...
                pageTitle: normalizeCustomerFacingBranding(
                  permutation.metadata.pageTitle,
                ),
                metaDescription: normalizeCustomerFacingBranding(
                  permutation.metadata.metaDescription,
                ),
              }
            : permutation.metadata,
          excerpts: Array.isArray(permutation.excerpts)
            ? permutation.excerpts.map((value) =>
                normalizeCustomerFacingBranding(value),
              )
            : permutation.excerpts,
          descriptions: Array.isArray(permutation.descriptions)
            ? permutation.descriptions.map((value) =>
                normalizeCustomerFacingBranding(value),
              )
...
function uniqueStrings(values) {
  return [
    ...new Set(
      values.filter(Boolean).map((value) => normalizeWhitespace(String(value))),
    ),
  ];
}
...
async function readLivewireSnapshots(page) {
  const rawSnapshots = await page
    .$$eval("[wire\\:snapshot]", (nodes) =>
      nodes
        .map((node) => node.getAttribute("wire:snapshot") || "")
        .filter(Boolean),
    )
    .catch(() => []);
...
function isLikelyModelLabel(label) {
  ...
  if (
    /necessary|preferences|statistics|marketing|unclassified|allow|deny|customize|playground|contact/i.test(
      normalized,
    )
  ) {
    return false;
...
async function readLivewirePricingState(page, category) {
  ...
  const pricingSnapshots = snapshots.filter((snapshot) =>
    String(snapshot?.memo?.name || "").startsWith("pricing"),
  );
  ...
  const target =
    pricingSnapshots.find(
      (snapshot) =>
        normalizeWhitespace(
          String(snapshot?.memo?.name || ""),
        ).toLowerCase() === `pricing.${category}`,
    ) || pricingSnapshots[0];
  ...
  if (Number.isFinite(fromExamples) && fromExamples > 0) {
    return {
      ...
      priceText: buildLivewirePriceText(
        category,
        "pricingExamples",
        fromExamples,
        data,
...
async function discoverModelOptions(page, sourceUrl) {
  ...
  const pricingSnapshot =
    snapshots.find((snapshot) => snapshot?.memo?.name === "pricing") || null;

  const fromSnapshot = extractModelsForTypeFromSnapshot(
    pricingSnapshot,
    inferredType,
  );
  ...
  const fromSelect = await page
    .$$eval(
      "select[wire\\:model.live='modelSlug'] option, select[wire\\:model='modelSlug'] option",
      (options) =>
        options.map((option) => ({
  ...
  const fromSidebar = await page
    .$$eval(
      '[wire\\:key^="sidebar-model-"], [wire\\:key^="mobile-model-"]',
      (buttons) =>
        buttons
          .map((button) => ({
            value: (
              (button.getAttribute("wire:key") || "").match(
                /(?:sidebar|mobile)-model-(.+)$/,
              )?.[1] || ""
            ).trim(),
  ...
  const url = new URL(sourceUrl);
  const modelFromQuery = normalizeWhitespace(
  ...
  return page
    .evaluate(() => {
      ...
      if (root) {
        try {
          selectedSlug =
            JSON.parse(root.getAttribute("wire:snapshot") || "{}")?.data
              ?.selectedModelSlug || "";
        } catch {
          selectedSlug = "";
        }
      ...
      if (selectedSlug && bySlug.has(selectedSlug)) {
        return {
          value: selectedSlug,
          label: bySlug.get(selectedSlug) || selectedSlug,
        };
      }
...
async function discoverPricingSourceUrls(page) {
  await page.goto(DEAPI_PRICING_BASE_URL, {
  ...
  const discoveredBySnapshot = inferredTypes.map(
    (type) => `${DEAPI_PRICING_BASE_URL}?type=${type}`,
  );
  ...
  const discoveredByButtons = discoveredTypesFromButtons
    .map((label) => {
      ...
      if (
        ...
        ["background-removal", "image-upscale"].includes(normalized)
      ) {
  ...
  return page
    .$$eval("input[type='range'], select", (inputs) =>
      inputs.map((input, index) => {
        ...
        const textFromClosestLabel = (
          input.closest("label")?.textContent || ""
        ).trim();
        const textFromLegend = (
          input.closest("fieldset")?.querySelector("legend")?.textContent || ""
        ).trim();
        const textFromPrevious = (
...
async function readPricingSignals(page) {
  return page.evaluate(() => {
    const nodes = Array.from(
      document.querySelectorAll(
        "main, section, article, div, p, span, h1, h2, h3, li",
      ),
    );
    ...
    const isNoise = (text) => {
      return /cookie|consent|privacy|marketing|advertis|allow|deny|show details|terms of service|all rights reserved|copyright/i.test(
        text,
      );
    };
    ...
    const scoreText = (text) => {
      let score = 0;
      if (
        /starting price|estimated cost|price calculator|sample costs?/i.test(
          text,
        )
      )
        score += 6;
      if (
        /per\s*1m|per\s*transformation|per\s*second|per\s*minute|per\s*image|per\s*video|per\s*request/i.test(
          text,
        )
      )
        score += 5;
      if (/price|cost|credit|billing|charged?/i.test(text)) score += 3;
      if (/free credits?|claim\s*\$|sign\s*up|get started|new\s/i.test(text))
        score -= 8;
    ...
    for (const node of nodes) {
      ...
      if (
        /price|cost|billing|model|parameter/i.test(text) &&
        excerpts.length < 12
      ) {
        excerpts.push(text);
      }
      ...
      if (
        /description|about|details|works|supports/i.test(text) &&
        descriptions.length < 12
      ) {
...
function normalizePermutationPrice(pricingSignals) {
  ...
  const precise = byScore.find((text) =>
    /estimated\s+cost|starting\s+price|transcribed\s+price/i.test(text),
  );
  const primaryPriceText =
    precise || byScore[0] || pricingSignals.priceTexts[0] || "";
...
function buildCatalog(snapshot) {
  ...
  for (const permutation of snapshot.permutations) {
    ...
    Object.keys(permutation.params || {}).forEach((key) =>
      parameterKeysByCategory[permutation.category].add(key),
    );
  }

  const normalizedModelsByCategory = Object.fromEntries(
    Object.entries(modelsByCategory).map(([category, values]) => [
      category,
  ...
  const normalizedParameterKeysByCategory = Object.fromEntries(
    Object.entries(parameterKeysByCategory).map(([category, values]) => [
      category,
      [...values].sort((a, b) => a.localeCompare(b)),
    ]),
  );
...
function renderCatalogTs(catalog) {
  ...
  const paramsByCategory = JSON.stringify(
    catalog.parameterKeysByCategory,
    null,
    2,
  );
...
async function maybePushSnapshot(snapshot) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Snapshot sync endpoint failed: ${response.status} ${details}`,
    );
  }
}
...
async function run() {
  ...
  const maxModelsPerCategory = readNumberEnv(
    "DEAPI_MAX_MODELS_PER_CATEGORY",
    0,
  );
  ...
  const requestedConcurrency = Math.max(
    1,
    Math.floor(readNumberEnv("DEAPI_CONCURRENCY", 4)),
  );
  ...
  try {
    ...
    try {
      if (sourceUrlsOverride.length > 0) {
        ...
        console.log(
          `[pricing-sync] using DEAPI_SOURCE_URLS override (${sourceUrlsToScrape.length} URLs)`,
        );
      } else {
        const discoveredSourceUrls =
          await discoverPricingSourceUrls(discoveryPage);
        sourceUrlsToScrape = uniqueStrings(
          [...DEFAULT_URLS, ...discoveredSourceUrls]
            .map((url) => normalizePricingSourceUrl(url))
            .filter(Boolean),
        );
        console.log(
          `[pricing-sync] discovered ${sourceUrlsToScrape.length} pricing source URLs`,
        );
      }
      ...
      for (const sourceUrl of sourceUrlsToScrape) {
        ...
        await discoveryPage.goto(sourceUrl, {
          waitUntil: "domcontentloaded",
          timeout: 45_000,
        });
        await discoveryPage.waitForTimeout(500);

        const discoveredModelOptions = await discoverModelOptions(
          discoveryPage,
          sourceUrl,
        );
        const modelOptions =
          maxModelsPerCategory > 0
            ? discoveredModelOptions.slice(0, maxModelsPerCategory)
    ...
    const workerCount = Math.max(
      1,
      Math.min(requestedConcurrency, tasks.length),
    );
    console.log(
      `[pricing-sync] prepared ${tasks.length} model tasks; running with ${workerCount} workers`,
    );
      ...
      try {
        while (canAcceptPermutation()) {
          ...
          try {
            ...
            const visibleControls = controlsRaw.filter(
              (control) => control.visible,
            );
            const baseControls =
              visibleControls.length > 0 ? visibleControls : controlsRaw;
            ...
            const controls = baseControls.map((control, controlIndex) => {
              ...
              let nextKey =
                candidateKeys[0] || toParamFallback(category, controlIndex);
            ...
            const combinations = cartesianProduct(
              axes,
              maxPermutationsPerModel,
            );
            console.log(
              `[pricing-sync][w${workerId}] controls=${controls.length} combinations=${combinations.length}`,
            );
            ...
            for (const combination of combinations) {
              ...
              const livewirePricing = await waitForLivewirePricingState(
                page,
                category,
              );
              const resolvedPriceUsd =
                livewirePricing && Number.isFinite(livewirePricing.priceUsd)
                  ? livewirePricing.priceUsd
                  : parsedPricing.priceUsd;
              const resolvedPriceText =
                livewirePricing?.priceText || parsedPricing.priceText;
              ...
              const modelValue =
                selectedModel.value || modelOption.value || "default";
              const modelLabel =
                selectedModel.label ||
                modelOption.label ||
                modelValue ||
              ...
              const accepted = acceptPermutation({
                ...
                descriptions: uniqueStrings(pricingSignals.descriptions).slice(
                  0,
                  8,
                ),
                scrapedAt: now,
  ...
  const snapshot = {
    ...
    metadata: {
      scraper: "scripts/sync-deapi-pricing.ts",
  ...
  await maybePushSnapshot(normalizedSnapshot).catch((error) => {
    console.warn(
      `Could not push pricing snapshot to sync endpoint: ${error instanceof Error ? error.message : String(error)}`,
    );
  });

  console.log(
    `Synced ${snapshot.permutations.length} dryAPI pricing permutations at ${syncedAt}`,
  );
}
