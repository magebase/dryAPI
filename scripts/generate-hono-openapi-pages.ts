...
async function fetchOpenApiDocument(url) {
  ...
  if (!openapi.openapi || !openapi.paths) {
    throw new Error("OpenAPI object is missing required fields: openapi, paths")
  }
...
async function resolveOpenApiDocument() {
  ...
  const tempPort = Number.parseInt(process.env.HONO_OPENAPI_TEMP_PORT ?? "8788", 10)
...
async function main() {
  ...
  await fs.writeFile(openApiJsonPath, `${JSON.stringify(openapi, null, 2)}\n`, "utf8")
  ...
  await generateFiles({
    ...
    name: (entry) => {
      ...
      return tagName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "general"
    },
    index: {
      url: (filePath) => {
      ...
      items: [
        {
          ...
          description: "Generated from the dryAPI/deAPI gateway schema.",
        },
      ],
    },
    frontmatter: (title, description, context) => {
      if (context.type === "tag") {
        const tagTitle = context.tag?.["x-displayName"] ?? context.tag?.name ?? title
        return {
    ...
    addGeneratedComment: "Auto-generated from Hono openapi route metadata and TypeBox validators.",
  })
