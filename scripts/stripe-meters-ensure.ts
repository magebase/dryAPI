...
const EVENT_SPECS = [
  {
    env: "STRIPE_METER_EVENT_AI_MODEL_CALL",
    fallback: "dryapi_ai_model_call",
    amountEnv: "STRIPE_METER_PRICE_AI_MODEL_CALL_UNIT_AMOUNT",
    defaultUnitAmount: "1",
  },
  {
    env: "STRIPE_METER_EVENT_MODERATION_MODEL_CALL",
    fallback: "dryapi_moderation_model_call",
    amountEnv: "STRIPE_METER_PRICE_MODERATION_MODEL_CALL_UNIT_AMOUNT",
    defaultUnitAmount: "1",
  },
  {
    env: "STRIPE_METER_EVENT_BREVO_SMS_SEND",
    fallback: "dryapi_brevo_sms_send",
    amountEnv: "STRIPE_METER_PRICE_BREVO_SMS_SEND_UNIT_AMOUNT",
    defaultUnitAmount: "1",
  },
  {
    env: "STRIPE_METER_EVENT_CAL_REQUEST",
    fallback: "dryapi_cal_request",
    amountEnv: "STRIPE_METER_PRICE_CAL_REQUEST_UNIT_AMOUNT",
    defaultUnitAmount: "1",
  },
  {
    env: "STRIPE_METER_EVENT_CLOUDFLARE_WORKER_REQUEST",
    fallback: "dryapi_cloudflare_worker_request",
    amountEnv: "STRIPE_METER_PRICE_CLOUDFLARE_WORKER_REQUEST_UNIT_AMOUNT",
    defaultUnitAmount: "1",
  },
  {
    env: "STRIPE_METER_EVENT_WORKFLOW_DISPATCH",
    fallback: "dryapi_workflow_dispatch",
    amountEnv: "STRIPE_METER_PRICE_WORKFLOW_DISPATCH_UNIT_AMOUNT",
    defaultUnitAmount: "1",
  },
  {
    env: "STRIPE_METER_EVENT_WORKFLOW_RUN",
    fallback: "dryapi_workflow_run",
    amountEnv: "STRIPE_METER_PRICE_WORKFLOW_RUN_UNIT_AMOUNT",
    defaultUnitAmount: "1",
  },
];
...
function buildEventSpecs() {
  return EVENT_SPECS.map((spec) => {
    ...
    const unitAmountDecimal =
      clean(process.env[spec.amountEnv]) || spec.defaultUnitAmount;
    return {
      ...spec,
      eventName,
      unitAmountDecimal,
...
async function ensureProduct(apiKey, { dryRun, projectKey, currency }) {
  ...
  const existing = products.find((item) => {
    ...
    return (
      metadata.project_key === projectKey && metadata.kind === "metered_usage"
    );
  });
...
async function ensurePrice(
  ...
  const prices = await listAll(
    apiKey,
    `/v1/prices?active=true&product=${encodeURIComponent(productId)}`,
  );
...
async function main() {
  ...
  const currency = (
    clean(process.env.STRIPE_METER_PRICE_CURRENCY) || "aud"
  ).toLowerCase();
