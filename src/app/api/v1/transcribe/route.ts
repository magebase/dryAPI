...
export async function POST(request: NextRequest) {
  const parsed = transcribeRequestSchema.safeParse(await request.json().catch(() => null))
  if (!parsed.success) {
    return errorResponse(400, "invalid_request", "Invalid transcribe request payload")
  }
