...
export async function POST(request: NextRequest) {
  ...
  if (!isCrmWorkflowAutomationsEnabled()) {
    return NextResponse.json(
      { error: "CRM workflow automation is disabled." },
      { status: 403 },
    );
  }
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { error: "Invalid workflow dispatch payload." },
      { status: 400 },
    );
  }
    ...
    return NextResponse.json(
      {
        ok: false,
        error:
          "Missing CRM_AUTOMATION_DISPATCH_URL (or WORKFLOW_CHAT_ESCALATION_WEBHOOK_URL fallback).",
      },
      { status: 400 },
    );
  }
