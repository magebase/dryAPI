    ...
    return NextResponse.json(
      {
        ...
        message: "INTERNAL_API_KEY is required for internal verification routes.",
      },
      { status: 501 },
    )
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!token) {
    return NextResponse.json({ error: "invalid_request", message: "token is required" }, { status: 400 })
  }
  ...
  try {
    ...
    if (!result.valid) {
      return NextResponse.json({ ok: false, valid: false, authorized: false }, { status: 401 })
    }

    if (!result.authorized) {
      return NextResponse.json({ ok: false, valid: true, authorized: false }, { status: 403 })
    }
