...
function shouldRequireTurnstile(): boolean {
  return (process.env.STRIPE_DEPOSIT_TURNSTILE_REQUIRED || "false").trim().toLowerCase() === "true"
}

function resolveRedirectUrls(input: z.infer<typeof checkoutRequestSchema>): { successUrl: string; cancelUrl: string } {
  const fallbackBookingUrl = input.calcomBookingUrl?.trim()
  const defaultSuccess = process.env.STRIPE_DEPOSIT_DEFAULT_SUCCESS_URL?.trim() || ""
  const defaultCancel = process.env.STRIPE_DEPOSIT_DEFAULT_CANCEL_URL?.trim() || ""

  const successUrl = input.successUrl?.trim() || fallbackBookingUrl || defaultSuccess
  const cancelUrl = input.cancelUrl?.trim() || fallbackBookingUrl || defaultCancel || successUrl

  if (!successUrl || !cancelUrl) {
    throw new Error("Missing checkout redirect URLs. Provide successUrl/cancelUrl, calcomBookingUrl, or STRIPE_DEPOSIT_DEFAULT_* env vars")
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const body = (await stripeResponse.json().catch(() => null)) as StripeCheckoutSessionResponse | null

    if (!stripeResponse.ok) {
      return NextResponse.json(
        {
          ok: false,
          error: body?.error?.message || `Stripe checkout session creation failed (${stripeResponse.status})`,
        },
        { status: 502 }
      )
    }
