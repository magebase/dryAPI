...
async function parseContactRequest(
  ...
  if (
    ...
    const parsed = contactApiRequestSchema.parse(
      contactApiFormDataSchema.parse(formData),
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const turnstileAction = isQuoteSubmission
      ? "quote_submit"
      : "contact_submit";
    ...
    try {
      ...
      const contactCandidate = siteConfig.contact.contactEmail
        .trim()
        .toLowerCase();
      quoteRecipientFallback =
        quoteCandidate.length > 0 ? quoteCandidate : null;
      contactRecipientFallback =
        contactCandidate.length > 0 ? contactCandidate : null;
    } catch {
    ...
    const recipient = resolveRecipientForQueue({
      ...
      queue: enquiryQueue,
      env: process.env,
      fallbackRecipient: isQuoteSubmission
        ? quoteRecipientFallback
        : contactRecipientFallback,
    });
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Inquiry recipient is not configured. Set CONTACT/QUOTE routing env vars.",
        },
        { status: 500 },
      );
    }
    ...
    try {
      ...
      emailResponse = await sendBrevoReactEmail({
        ...
        tags: [
          "website",
          isQuoteSubmission ? "quote" : "contact",
          enquiryQueue,
        ],
        attachments,
      });
      ...
      return NextResponse.json(
        { ok: false, error: "Unable to send inquiry email right now" },
        { status: 502 },
      );
    }
    ...
    return NextResponse.json({
      ...
      id:
        ...
        createPublicId(isQuoteSubmission ? "quote" : "contact"),
      message: quotePersistenceFailed
        ? "Quote request sent, but archival storage is temporarily unavailable."
        : undefined,
    });
  } catch (error) {
