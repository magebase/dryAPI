...
export async function GET(
  ...
  const range = parseJsonParam<[number, number]>(request.nextUrl.searchParams.get("range"), [0, 24])
...
export async function POST(
  ...
  try {
    ...
    const record = await createCmsResourceRecord(resource, getPayload(rawPayload))
