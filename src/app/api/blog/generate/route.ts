  ...
  if (!configuredSecret) {
    if (process.env.NODE_ENV === "production") {
      return NextResponse.json(
        { ok: false, error: "BLOG_GENERATOR_SECRET must be configured in production." },
        { status: 500 }
      )
    }
...
async function readExistingSlugs(): Promise<Set<string>> {
  ...
  return new Set(fileNames.filter((name) => name.endsWith(".json")).map((name) => name.replace(/\.json$/, "")))
}
...
async function callGeminiModel({
  ...
  return {
    ...
    seoTitle: normalizeText(draft.seoTitle, "Brisbane Generator Hire And Service Guide"),
...
export async function POST(request: NextRequest) {
  ...
  const userThemes = parsedRequest.focusKeywords?.filter((value) => value.trim().length > 0) ?? []
  ...
  for (let index = 0; index < parsedRequest.count; index += 1) {
    ...
    const publishedAt = toDateString(new Date(Date.now() - index * 24 * 60 * 60 * 1000))
    ...
    try {
      ...
      if (!parsedRequest.dryRun) {
        ...
        created.push({ slug: post.slug, title: post.title, filePath, publishedAt: post.publishedAt })
      } else {
        created.push({ slug: post.slug, title: post.title, filePath: "(dry-run)", publishedAt: post.publishedAt })
      }
    } catch (error) {
      errors.push({
        index,
