...
const siteFixtureWithAddress: SiteConfig = {
  ...
  footer: {
    ...
    contactLinks: [
      ...
      {
        label: "Email",
        href: "mailto:sales@genfix.com.au",
      },
      {
    ...
    socialLinks: [
      {
        label: "Primary",
        href: "https://example.com/company/genfix",
        icon: "linkedin",
      },
      {
        label: "Secondary",
        href: "http://example.net/genfix",
        icon: "youtube",
      },
      {
        label: "Duplicate",
      ...
      {
        label: "Ignored",
        href: "ftp://example.com/company/genfix",
        icon: "linkedin",
      },
    ],
    columns: [],
...
const siteFixtureWithoutAddress: SiteConfig = {
  ...
  footer: {
    ...
    socialLinks: [
      {
        label: "Ignored",
        href: "mailto:team@genfix.com.au",
        icon: "linkedin",
      },
    ],
  },
...
vi.mock("@/components/site/site-header", () => ({
  SiteHeader: ({ pathname }: { pathname: string }) => <div data-pathname={pathname} data-testid="site-header" />,
}))
...
vi.mock("@/components/site/ai-sales-chat-widget", () => ({
  AiSalesChatWidget: ({ pathname }: { pathname: string }) => <div data-pathname={pathname} data-testid="chat-widget" />,
}))
...
vi.mock("@/components/site/plan-tier-clarity-slot", () => ({
  ...
  }) => <div data-clarity-project-id={clarityProjectId} data-plan-tier={planTier} data-testid="clarity-slot" />,
}))
...
describe("SiteFrame", () => {
  it("wires child widgets and JSON-LD props from resolved site values", () => {
    ...
    expect(screen.getByTestId("site-header")).toHaveAttribute("data-pathname", "/hire")
    expect(screen.getByTestId("site-footer")).toBeInTheDocument()
    expect(screen.getByTestId("pwa-install-cta")).toBeInTheDocument()
    expect(screen.getByTestId("page-content")).toBeInTheDocument()

    expect(screen.getByTestId("chat-widget")).toHaveAttribute("data-pathname", "/hire")
    expect(screen.getByTestId("clarity-slot")).toHaveAttribute("data-plan-tier", "growth")
    expect(screen.getByTestId("clarity-slot")).toHaveAttribute("data-clarity-project-id", "clarity-123")
    ...
    expect(sameAs).toEqual(["https://example.com/company/genfix", "http://example.net/genfix"])

    expect(localBusinessJsonLdProps?.address).toBe(siteFixtureWithAddress.footer.contactLinks[2]?.label)
  })
  ...
  it("falls back to safe defaults when path, address, and env inputs are missing", () => {
    ...
    expect(screen.getByTestId("site-header")).toHaveAttribute("data-pathname", "/")
    expect(screen.getByTestId("chat-widget")).toHaveAttribute("data-pathname", "/")
