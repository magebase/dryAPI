...
export function ContactPageTemplate({ page, site }: ContactPageTemplateProps) {
  const contactPanelHeading = page.contactPanel?.heading ?? "Contact Us"
  ...
  const locationFallback = site.footer.columns
    ...
    ?.links.map((link, index) => ({
      ...
      title: link.label,
      description: `${link.label} branch support and service coordination available on request.`,
      href: link.href,
      ...
      <section className="bg-[#262f3d] py-12 md:py-20">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 lg:grid-cols-[0.9fr_1.1fr]">
          <Reveal as="div" className={`${getGradientVariant(3)} rounded-md border border-white/10 p-6`}>
            ...
            <p className="mt-3 text-sm text-slate-300">For critical jobs and shutdown timelines, direct phone contact is the fastest path.</p>
            <QuoteAwareLink
          ...
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.22em] text-slate-100">Our Locations</p>
            <p className="mt-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-300">Nation-wide support</p>
