...
const quoteFormSchema = z.object({
  ...
  projectSuburbPostcode: z.string().trim().min(1, "Postcode/suburb of project is required."),
  ...
  deliveryDetails: z.string().trim().min(1, "Please add your delivery details."),
  notes: z.string().trim(),
})
...
export function QuoteDialog({ site, triggerLabel, triggerClassName, triggerTinaField }: QuoteDialogProps) {
  ...
  const dialogDescription = resolveSiteUiText(site, "quoteDialog.description", "Share a few project details and our team will get back to you quickly.")
  ...
  const fieldEnquiry = resolveSiteUiText(site, "quoteDialog.field.enquiry", "Are you enquiring about rental or sales?*")
  const fieldInterest = resolveSiteUiText(site, "quoteDialog.field.interest", "What are you interested in?*")
  const fieldDelivery = resolveSiteUiText(site, "quoteDialog.field.delivery", "Do you need delivery? (If yes please enter suburb/postcode, if no move to next)*")
  ...
  const submitMutation = useMutation({
    mutationFn: async (values: QuoteFormValues) => {
      ...
      const formData = buildContactFormData(payload as Record<string, string>, files, turnstileToken)
    ...
    onError: (mutationError) => {
      const message = mutationError instanceof Error ? mutationError.message : submitError.value
      setStatusTone("error")
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="z-[60] max-h-[88vh] overflow-y-auto border-slate-200 bg-white p-6 text-site-strong sm:max-w-[46rem]">
        <DialogHeader className="space-y-1">
          <DialogTitle className="font-display text-2xl uppercase tracking-[0.06em] text-[#101a28]" data-tina-field={dialogTitle.field}>
            {dialogTitle.value}
          </DialogTitle>
          <DialogDescription className="text-sm text-slate-600" data-tina-field={dialogDescription.field}>
        ...
        >
          <p className="text-xs uppercase tracking-[0.15em] text-site-muted" data-tina-field={requiredHint.field}>
            {requiredHint.value}
          </p>

          <div className="grid gap-3 sm:grid-cols-2">
            <form.Field name="firstName" children={(field) => renderTextField(field, fieldFirstName, "quote-first-name")} />
            <form.Field name="lastName" children={(field) => renderTextField(field, fieldLastName, "quote-last-name")} />
          </div>

          <div className="grid gap-3 sm:grid-cols-2">
            <form.Field name="email" children={(field) => renderEmailField(field, fieldEmail, "quote-email")} />
            <form.Field name="companyName" children={(field) => renderTextField(field, fieldCompany, "quote-company")} />
          </div>

          <div className="grid gap-3 sm:grid-cols-2">
            <form.Field name="phoneNumber" children={(field) => renderTextField(field, fieldPhone, "quote-phone")} />
            <form.Field
              name="projectSuburbPostcode"
              children={(field) => renderTextField(field, fieldPostcode, "quote-postcode")}
            />
          </div>
          ...
          <form.Field
            ...
            children={(field) => (
              <div className="space-y-2">
                <Label className="text-[11px] font-semibold uppercase tracking-[0.13em] text-slate-600" htmlFor={field.name} data-tina-field={fieldEnquiry.field}>
                  {fieldEnquiry.value}
                </Label>
                <select
          ...
          <form.Field
            ...
            children={(field) => (
              <div className="space-y-2">
                <Label className="text-[11px] font-semibold uppercase tracking-[0.13em] text-slate-600" htmlFor={field.name} data-tina-field={fieldInterest.field}>
                  {fieldInterest.value}
                </Label>
                <select
                  className="h-10 w-full rounded-sm border border-slate-300 bg-white px-3 text-sm text-site-strong outline-none ring-primary transition focus:ring-2"
          ...
          <form.Field
            ...
            children={(field) => renderTextField(field, fieldDelivery, "quote-delivery")}
          />

          <form.Field name="notes" children={(field) => renderTextareaField(field, fieldNotes, "quote-notes")} />
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
            <p className="text-xs text-site-muted" data-tina-field={responseTime.field}>
              {responseTime.value}
            </p>
            <Button
              ...
              type="submit"
            >
              {submitMutation.isPending || form.state.isSubmitting ? submitBusy.value : submitIdle.value}
            </Button>
          </div>
          ...
          {statusMessage ? (
            <p
              className={cn(
                "text-sm",
                statusTone === "error" ? "text-primary" : statusTone === "success" ? "text-emerald-700" : "text-slate-700",
              )}
            >
              {statusMessage}
            </p>
  ...
  function renderTextField(
    field: { name: string; state: { value: string; meta: { isTouched: boolean; isValid: boolean; errors: unknown[] } }; handleBlur: () => void; handleChange: (value: string) => void },
    ...
    return (
      <div className="space-y-2">
        <Label className="text-[11px] font-semibold uppercase tracking-[0.13em] text-slate-600" htmlFor={id} data-tina-field={label.field}>
          {label.value}
        </Label>
        <Input
