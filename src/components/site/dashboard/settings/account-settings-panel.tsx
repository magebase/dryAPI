    ...
    async function loadSession() {
      try {
        ...
        if (!sessionResponse.ok || !sessionsResponse.ok) {
          throw new Error(`Failed to load session (${sessionResponse.status}/${sessionsResponse.status})`)
        }

        const payload = (await sessionResponse.json().catch(() => null)) as { user?: SessionUser | null } | null
        const sessionsPayload = (await sessionsResponse.json().catch(() => null)) as SessionRecord[] | null

        if (active) {
          setUser(payload?.user ?? null)
          setActiveSessions(Array.isArray(sessionsPayload) ? sessionsPayload : [])
          setLoadError(null)
        }
      } catch {
        if (active) {
  ...
  async function handleDeleteRequest() {
    ...
    try {
      ...
      if (response.status === 409) {
        toast.error(payload?.message || "Account deletion is blocked while balance is negative.")
        return
      }

      if (!response.ok) {
        toast.error(payload?.message || "Unable to submit delete account request.")
        return
      }

      toast.info("Delete account request", {
        description: payload?.next || "Contact support@dryapi.ai to complete account deletion.",
      })
    } catch {
    ...
    <div className="space-y-5" aria-busy={loading}>
      <div className="grid gap-4 rounded-lg border border-zinc-200/80 bg-zinc-50/70 p-4 dark:border-zinc-700/80 dark:bg-zinc-800/40 md:grid-cols-2">
        <div>
          ...
          {loading ? (
            ...
            <>
              <p className="mt-1 text-sm font-medium text-zinc-900 dark:text-zinc-100">{user?.name || "Unnamed account"}</p>
              <p className="text-xs text-zinc-500 dark:text-zinc-400">{user?.email || "No email available"}</p>
            </>
          )}
        </div>
        ...
        <div>
          <p className="text-xs uppercase tracking-[0.14em] text-zinc-500 dark:text-zinc-400">Role</p>
          <div className="mt-1 flex items-center gap-2">
        ...
        <div>
          <p className="text-xs uppercase tracking-[0.14em] text-zinc-500 dark:text-zinc-400">Last sign-in</p>
      ...
      <div className="rounded-lg border border-zinc-200/80 bg-white p-4 dark:border-zinc-700/80 dark:bg-zinc-900/70">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <Button type="button" variant="outline" onClick={handleSignOutOtherSessions} disabled={signOutOthersPending}>
            {signOutOthersPending ? "Revoking..." : `Sign out other sessions (${Math.max(activeSessions.length - 1, 0)})`}
          </Button>
        ...
        <div className="mt-4 rounded-lg border border-zinc-200/80 bg-zinc-50/70 p-3 dark:border-zinc-700/80 dark:bg-zinc-800/40">
          <p className="text-xs uppercase tracking-[0.14em] text-zinc-500 dark:text-zinc-400">Active sessions</p>
          <p className="mt-1 text-sm font-medium text-zinc-900 dark:text-zinc-100">{activeSessions.length}</p>
          <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
            Better Auth is currently tracking your active browser or device sessions for this account.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-lg border border-red-200/80 bg-red-50/70 p-4 dark:border-red-900/40 dark:bg-red-900/20">
        ...
        >
          {deleteRequestPending ? "Submitting request..." : "Request account deletion"}
        </Button>
      </div>
    </div>
  )
