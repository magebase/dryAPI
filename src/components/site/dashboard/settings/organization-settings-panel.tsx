...
): Array<(typeof ORGANIZATION_ROLES)[number]> {
  return (value || "")
    .split(",")
      ...
      <div className="space-y-3 rounded-lg border border-zinc-200/80 bg-white p-4 dark:border-zinc-700/80 dark:bg-zinc-900/70">
        {Array.from({ length: 2 }, (_, index) => (
          <div
            ...
            className="flex items-center justify-between gap-4 rounded-lg border border-zinc-200/80 p-3 dark:border-zinc-700/80"
          >
      ...
      <div className="space-y-3 rounded-lg border border-zinc-200/80 bg-white p-4 dark:border-zinc-700/80 dark:bg-zinc-900/70">
        {Array.from({ length: 2 }, (_, index) => (
          <div
            ...
            className="flex items-center justify-between gap-3 rounded-lg border border-zinc-200/80 p-3 dark:border-zinc-700/80"
          >
            <div className="space-y-2">
              <Skeleton className="h-4 w-40" />
...
export function OrganizationSettingsPanel() {
  ...
  const [memberRoleUpdates, setMemberRoleUpdates] = useState<
  ...
  const [userInvitationActionId, setUserInvitationActionId] = useState<
    string | null
  >(null);
  ...
  const createOrganizationMutation = useMutation({
    ...
    onSuccess: (payload) => {
      ...
      toast.success(
        payload?.name
          ? `Workspace ${payload.name} created`
          : "Workspace created",
      );
    },
    onError: (error) => {
  ...
  useEffect(() => {
    ...
    async function loadOrganizations() {
      try {
        ...
        const organizationsPayload = (await orgResponse
          .json()
        ...
        const nextOrganizations = Array.isArray(organizationsPayload)
          ? organizationsPayload
          : [];
        const sessionUser =
          (sessionSnapshot.user as SessionPayload["user"] | null) ?? null;
        const sessionRecord =
          (sessionSnapshot.session as SessionPayload["session"] | null) ?? null;
  ...
  async function handleUpdateMemberRole(memberId: string) {
    ...
    if (
      ...
      !hasOrganizationManagementRole(activeMembership?.role)
    ) {
      return;
    }
  ...
  const canManageOrganizationMembers =
    ...
    hasOrganizationManagementRole(activeMember?.role);
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3 rounded-lg border border-zinc-200/80 bg-white p-4 dark:border-zinc-700/80 dark:bg-zinc-900/70">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
            Use Better Auth organizations to separate billing context, members,
            and API key ownership.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-3 rounded-lg border border-zinc-200/80 p-3 dark:border-zinc-700/80 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400">
              Uses your personal billing profile, API keys, and account-level
              settings.
            </p>
          </div>
          ...
          <Button
            ...
            disabled={
              isPersonalWorkspaceActive ||
              switchingId === PERSONAL_WORKSPACE_SWITCH_KEY
            }
        ...
        {organizations.length > 0 ? (
          ...
          <div className="rounded-lg border border-dashed border-zinc-300/80 p-4 text-sm text-zinc-500 dark:border-zinc-700/80 dark:text-zinc-400">
            No workspaces created yet. Create one below to start using
            organization-scoped auth features.
          </div>
        )}
      </div>
      ...
      >
          ...
          children={(field) => {
            const isInvalid =
              (field.state.meta.isTouched ||
                createOrganizationForm.state.isSubmitted) &&
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) => {
                    ...
                    if (!slugTouched) {
                      createOrganizationForm.setFieldValue(
                        "slug",
                        toSlug(nextValue),
                      );
                    }
        ...
        <div className="md:col-span-2 flex items-center justify-between gap-3 border-t border-zinc-200/80 pt-4 dark:border-zinc-700/80">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            Creating a workspace also sets it as your primary workspace by
            default.
          </p>
          <Button
            type="submit"
            disabled={
              ...
              createOrganizationForm.state.isSubmitting
            }
          >
            {createOrganizationMutation.isPending ||
            createOrganizationForm.state.isSubmitting
      ...
      >
        <div className="md:col-span-3 flex items-center justify-between gap-3 border-b border-zinc-200/80 pb-3 dark:border-zinc-700/80">
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary">
              Your role: {describeOrganizationRole(activeMember?.role)}
            </Badge>
          </div>
        </div>
        ...
        <inviteMemberForm.Field
          ...
          children={(field) => {
            const isInvalid =
              (field.state.meta.isTouched ||
                inviteMemberForm.state.isSubmitted) &&
              !field.state.meta.isValid;
            const errorMessage = getValidationErrorMessage(
              field.state.meta.errors[0],
            );
        ...
        <div className="md:col-span-3 flex justify-end border-t border-zinc-200/80 pt-4 dark:border-zinc-700/80">
          <Button
            ...
            disabled={
              ...
              inviteMemberForm.state.isSubmitting
            }
          >
            {inviteMemberMutation.isPending ||
            inviteMemberForm.state.isSubmitting
        ...
        {members.length > 0 ? (
          members.map((member) => {
            ...
            return (
              <div
                key={member.id}
                className="flex flex-col gap-3 rounded-lg border border-zinc-200/80 p-3 dark:border-zinc-700/80 lg:flex-row lg:items-center lg:justify-between"
              >
                <div>
                  <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  <Select
                    value={normalizeOrganizationRole(
                      memberRoleUpdates[member.id],
                    )}
                    onValueChange={(value) =>
      ...
      <div className="space-y-3 rounded-lg border border-zinc-200/80 bg-white p-4 dark:border-zinc-700/80 dark:bg-zinc-900/70">
        ...
        {userInvitations.length > 0 ? (
          userInvitations.map((invitation) => {
            ...
            return (
              <div
                key={`user-invitation-${invitation.id}`}
                className="flex flex-col gap-3 rounded-lg border border-zinc-200/80 p-3 dark:border-zinc-700/80 md:flex-row md:items-center md:justify-between"
              >
                <div>
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    onClick={() =>
                      void handleRespondToInvitation(invitation.id, "reject")
                    }
                  >
                    {userInvitationActionId === rejectActionId
                      ? "Rejecting..."
                      : "Reject"}
                  </Button>
                  <Button
                    ...
                    disabled={Boolean(userInvitationActionId)}
                    onClick={() =>
                      void handleRespondToInvitation(invitation.id, "accept")
                    }
                  >
                    {userInvitationActionId === acceptActionId
                      ? "Accepting..."
                      : "Accept"}
                  </Button>
                </div>
