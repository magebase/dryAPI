...
const STRIPE_METER_EVENT_DEFAULTS: Record<StripeMeterEventType, string> = {
  ai_model_call: "dryapi_ai_model_call",
  moderation_model_call: "dryapi_moderation_model_call",
  brevo_sms_send: "dryapi_brevo_sms_send",
  cal_request: "dryapi_cal_request",
...
function resolveMeterConfig(
  env: NodeJS.ProcessEnv = process.env,
): StripeMeterConfig | null {
...
function resolveStripeBrandKey(
  inputMetadata: Record<string, StripeMeterMetadataValue> | undefined,
): string {
  ...
  const fromEnv = (
    process.env.SITE_BRAND_KEY ||
    process.env.DRYAPI_BRAND_KEY ||
    "dryapi"
  )
    .trim()
    .toLowerCase();
...
function buildIdentifier(input: {
  ...
  return `${input.projectKey}:${input.eventType}:${input.timestamp}:${randomUUID()}`.slice(
    0,
    200,
  );
}
...
export async function recordStripeMeterUsage(
  input: RecordStripeMeterUsageInput,
): Promise<boolean> {
  ...
  params.set(
    ...
    resolveStripeBrandKey(input.metadata),
  );
