 ...
 */
export function resolvePlanMonthlyCredits(monthlyPriceUsd: number, discountPercent: number): number {
...
const SAAS_PLANS: readonly SaasPlanDefinition[] = [
  ...
  },
  {
    slug: "scale",
    label: "Scale",
    description: "For high-throughput deployments with premium support expectations.",
...
export function resolveSaasPlan(value: string | null | undefined): SaasPlanDefinition | null {
...
export function resolveSaasPlanStripePriceId(plan: SaasPlanDefinition, env: NodeJS.ProcessEnv = process.env): string | null {
  const priceId = env[plan.stripePriceIdEnvKey]?.trim() || ""
  return priceId.length > 0 ? priceId : null
}

export function resolveSaasPlanDiscountCents(inputAmountCents: number, discountPercent: number): number {
...
export function resolveMonthlyTokenExpiryIso(referenceDate: Date = new Date()): string {
...
export function resolveCurrentMonthlyTokenCycleStartIso(referenceDate: Date = new Date()): string {
  const year = referenceDate.getUTCFullYear()
  const month = referenceDate.getUTCMonth()
...
export function resolveAnnualMonthlyPriceUsd(plan: SaasPlanDefinition): number {
  return Number((plan.monthlyPriceUsd * (1 - plan.annualDiscountPercent / 100)).toFixed(2))
}
...
export function resolveAnnualSavingsUsd(plan: SaasPlanDefinition): number {
  return Number((plan.monthlyPriceUsd * 12 - resolveAnnualPriceUsd(plan)).toFixed(2))
}
 ...
 */
export function resolveSaasPlanStripeAnnualPriceId(plan: SaasPlanDefinition, env: NodeJS.ProcessEnv = process.env): string | null {
