...
describe("runpod-profit-optimizer", () => {
  it("loads configured deAPI model profiles", () => {
  ...
  it("recommends endpoint autoscaling and gpu fallback order", () => {
    ...
    expect(recommendation.gpuFallbackOrder).toEqual(["rtx4090", "a6000", "a100"])
    expect(recommendation.autoscaling).toEqual({ minWorkers: 0, maxWorkers: 3 })
  })
