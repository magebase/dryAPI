...
describe("stripe deposit checkout helpers", () => {
  ...
  it("rejects invalid major-unit amounts", () => {
    ...
    expect(() => parseDepositAmountToCents("1000")).toThrowError(/cannot exceed/i)
  })
  ...
  it("rejects invalid auto top-up thresholds", () => {
    expect(() => parseAutoTopUpThresholdToCents(-1)).toThrowError(/zero or higher/i)
    expect(() => parseAutoTopUpThresholdToCents("abc")).toThrowError(/positive number/i)
  })

  it("recognizes preset top-up amounts", () => {
    expect(isPresetCreditTopUpAmountCents(CREDIT_TOP_UP_PRESET_AMOUNTS_CENTS[0])).toBe(true)
  ...
  it("omits optional checkout params when they are blank", () => {
    ...
    expect(payload.get("payment_intent_data[statement_descriptor_suffix]")).toBeNull()
