...
async function readSourceRows(
  db: D1DatabaseLike,
): Promise<{ leads: QuoteLeadRow[]; moderation: ModerationRow[] }> {
...
function buildTrend(leads: CrmLead[]): CrmTrendPoint[] {
  ...
  for (let index = 13; index >= 0; index -= 1) {
    ...
    buckets.set(day, {
      day,
      label: now.toLocaleDateString("en-AU", {
        day: "2-digit",
        month: "short",
      }),
      newLeads: 0,
...
function buildQueueBreakdown(leads: CrmLead[]): CrmQueueBreakdown[] {
  ...
  return Array.from(grouped.entries())
    .map(([queue, stats]) => ({
      ...
      count: stats.count,
      avgScore:
        stats.count > 0 ? Math.round(stats.scoreTotal / stats.count) : 0,
    }))
...
function buildChatHistory(
  ...
  const moderationEvents: CrmHistoryEvent[] = moderation
    .slice(0, 8)
    .map((entry) => {
...
export async function getCrmDashboardData(): Promise<CrmDashboardData> {
  ...
  const leadsLast24h = leads.filter(
    (lead) => Date.now() - new Date(lead.createdAt).getTime() <= 86_400_000,
  ).length;
  const avgLeadScore =
    leads.length > 0
      ? Math.round(
          leads.reduce((accumulator, lead) => accumulator + lead.score, 0) /
            leads.length,
        )
      : 0;
  const workflowKinds = isCrmWorkflowAutomationsEnabled()
    ? resolveEnabledWorkflowKinds()
    : [];
