...
const SALES_PATTERN = /\b(sale|sales|buy|purchase|owned|ownership|new\s+generator)\b/i
const RENTALS_PATTERN = /\b(rental|rent|hire|hiring|temporary|short\s*term)\b/i
const SERVICING_PATTERN = /\b(service|servicing|maintenance|repair|load\s*test|inspection|compliance)\b/i
const PARTS_PATTERN = /\b(parts?|spares?|filter|battery|alternator|controller|starter)\b/i
...
export function inferEnquiryQueue({
  ...
  const haystack = `${clean(enquiryType)} ${clean(message)} ${clean(question)}`.toLowerCase()

  if (enquiry.includes("part") || PARTS_PATTERN.test(haystack)) {
    return "parts"
  }

  if (enquiry.includes("rent") || enquiry.includes("hire") || RENTALS_PATTERN.test(haystack)) {
    return "rentals"
  }
...
export function resolveRecipientForQueue({
  ...
  if (queue !== "general") {
    ...
    const globalSpecific = parseEnvEmail(env, `GENFIX_EMAIL_${suffix}_TO`)
    if (globalSpecific) {
  ...
  const legacyChatDefault = channel === "chat" ? parseEnvEmail(env, "CHAT_ESCALATION_EMAIL_TO") : null
  if (legacyChatDefault) {
    return legacyChatDefault
  }

  const globalDefault = parseEnvEmail(env, "GENFIX_EMAIL_DEFAULT_TO")
  if (globalDefault) {
...
export function resolveFromNameForChannel({
  ...
  if (channel === "contact") {
    return clean(env.BREVO_FROM_NAME_CONTACT) || clean(env.BREVO_FROM_NAME) || "GenFix"
  }

  if (channel === "quote") {
    return clean(env.BREVO_FROM_NAME_QUOTE) || clean(env.BREVO_FROM_NAME) || "GenFix"
  }

  return clean(env.BREVO_FROM_NAME_CHAT) || clean(env.BREVO_FROM_NAME) || "GenFix"
}
