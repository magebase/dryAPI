...
const TINA_EDITOR_TOKEN_ISSUER = "dryapi";
...
function parseAllowlist(): ParsedAllowlist {
  const emails = new Set(
    parseCsv(process.env.TINA_ALLOWED_GOOGLE_EMAILS).map(normalizeEmail),
  );
  const domains = new Set(
    parseCsv(process.env.TINA_ALLOWED_GOOGLE_DOMAINS).map(normalizeDomain),
  );
...
export async function signTinaEditorToken(
  user: BetterAuthUser,
): Promise<string | null> {
  if (!user.email) {
    return null;
...
export async function verifyTinaEditorToken(
  token: string,
...
export async function getBetterAuthSession(request: Request): Promise<{
  ...
  const session = (await response
    .json()
    .catch(() => null)) as BetterAuthSessionResponse;
  return {
    session,
    setCookieHeader,
  };
