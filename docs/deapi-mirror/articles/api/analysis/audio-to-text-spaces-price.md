...
````yaml openapi.json post /api/v1/client/aud2txt/price-calculation
openapi: 3.0.0
info:
  ...
  /api/v1/client/aud2txt/price-calculation:
    post:
      tags:
        - Client API
      description: >-
        Endpoint for calculating price for audio to text inference (Twitter
        Spaces)
      operationId: requestPriceForAud2Text
      parameters:
          ...
          application/json:
            schema:
              ...
              properties:
                audio_url:
                  description: >-
                    URL of Twitter Spaces audio to transcribe (required if
                    duration_seconds not provided)
                  type: string
                  example: https://twitter.com/i/spaces/1nAKEERkeLbKL
                duration_seconds:
                  description: Duration in seconds (required if audio_url not provided)
                  type: number
                  example: 3600
                include_ts:
      ...
      responses:
        '200':
          description: Calculated price for audio to text inference.
          content:
            application/json:
              schema:
                properties:
