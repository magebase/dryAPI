...
````yaml openapi.json post /api/v1/client/img-rmbg/price-calculation
openapi: 3.0.0
info:
...
paths:
  /api/v1/client/img-rmbg/price-calculation:
    post:
      tags:
        - Client API
      description: Endpoint for calculating price for image background removal inference
      operationId: requestPriceForImgRemoveBg
      parameters:
        - $ref: '#/components/parameters/AcceptHeader'
      requestBody:
        description: Image background removal parameters
        required: true
        content:
          multipart/form-data:
            schema:
              oneOf:
                - required:
                  ...
                  properties:
                    image:
                      description: >-
                        Image file to remove background from. Supported formats:
                        JPG, JPEG, PNG, GIF, BMP, WebP. Maximum file size: 10
                        MB.
                      type: string
                      format: binary
                    model:
      ...
      responses:
        '200':
          description: Calculated price for image background removal inference.
          content:
            application/json:
              schema:
                properties:
