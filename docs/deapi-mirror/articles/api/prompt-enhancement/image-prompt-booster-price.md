...
````yaml openapi.json post /api/v1/client/prompt/image/price-calculation
openapi: 3.0.0
info:
...
paths:
  /api/v1/client/prompt/image/price-calculation:
    post:
      tags:
        - Client API
      description: Calculate the price for enhancing text-to-image prompts
      operationId: clientCalculateText2ImagePromptBoosterPrice
      requestBody:
        required: true
        content:
          application/json:
