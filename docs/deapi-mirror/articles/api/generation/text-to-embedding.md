...
````yaml openapi.json post /api/v1/client/txt2embedding
openapi: 3.0.0
info:
...
paths:
  /api/v1/client/txt2embedding:
    post:
      tags:
        - Client API
      description: Endpoint for requesting text to embedding inference
      operationId: requestTxt2Embedding
      parameters:
        - $ref: '#/components/parameters/AcceptHeader'
      requestBody:
        description: Text to embedding conversion parameters
        required: true
        content:
          application/json:
            schema:
              ...
              properties:
                input:
                  description: >-
                    Input text(s) to generate embeddings for. Can be a single
                    string or array of strings (max 2048 items). Each input
                    limited to 8192 tokens, total request limited to 300k
                    tokens.
                  oneOf:
                    - type: string
                ...
                model:
                  description: >-
                    The embedding model to use. Available models can be
                    retrieved via the GET /api/v1/client/models endpoint.
                  type: string
                  example: Bge_M3_FP16
                return_result_in_response:
                  description: >-
                    If true, the embedding result will be returned directly in
                    the response instead of only download url. Optional
                    parameter.
                  type: boolean
                  default: false
                  example: false
