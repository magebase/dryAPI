...
for (const plan of plans) {
    // Create Product
    const product = new stripe.Product(`plan-${plan.slug}`, {
        name: plan.name,
        active: true,
        metadata: {
            plan_slug: plan.slug,
    ...
    const monthlyPrice = new stripe.Price(`price-${plan.slug}-monthly`, {
        product: product.id,
    ...
    // Create Annual Price
    const annualPrice = new stripe.Price(`price-${plan.slug}-annual`, {
        product: product.id,
...
const portalConfig = new stripe.PortalConfiguration("default-portal", {
    businessProfile: {
        headline: "GenFix Consultancy Plans",
        privacyPolicyUrl: "https://genfix.com.au/privacy-policy",
        termsOfServiceUrl: "https://genfix.com.au/terms",
    },
    features: {
        customerUpdate: {
            enabled: true,
